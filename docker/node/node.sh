...
# docker run --rm --name node -e NODE_ENV=production node:13-alpine node -v

# 2. run server in Docker
# docker run --rm -d -it --name node -v $(pwd):/usr/src/app -p 80:8080 -e PORT=8080 -e SERVER_NAME=Docker -w /usr/src/app node:13-alpine node server.js

# 3. run npm script in Docker
docker run --rm -dit --name node -v $(pwd):/usr/src/app -p 80:8080 -e PORT=8080 -e SERVER_NAME=Docker -w /usr/src/app node:13-alpine npm start
